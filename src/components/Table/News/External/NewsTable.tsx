...
import { INewsExternalTable, IPagination, ISearchNews } from "~/interface";
...
const NewsTable = (props: Props) => {
  ...
  const [listItem, setListItem] = useState<INewsExternalTable[]>([]);
  ...
  const updateList = (
    record: INewsExternalTable,
    newData: Partial<INewsExternalTable>,
  ) => {
  ...
  const onActive = async (newsId: string, record: INewsExternalTable) => {
    if (!newsId) return;
  ...
  const onDisable = async (newsId: string, record: INewsExternalTable) => {
    if (!newsId) return;
  ...
  // columnt table
  const columns: TableColumnsType<INewsExternalTable> = useMemo(() => {
    return [
      {
        title: t("table.id"),
        dataIndex: "id",
      ...
      },
      {
        ...
        dataIndex: "status",
        render: (value: boolean, record: INewsExternalTable) => (
          <div
            onClick={(e) => {
              e.stopPropagation();
            }}
      ...
      },
      {
        ...
        dataIndex: "action",
        render: (_, record: INewsExternalTable) => (
          <div className="flex items-center justify-center gap-2">
            <Button
              onClick={(e) => {
                e.stopPropagation();
                router.push(`/news/${record.id}/external`);
              }}
    ...
    <div className="bg-white rounded-lg">
      ...
      <div className="scrollHidden overflow-x-auto">
        <Table
          ...
          onRow={(record: INewsExternalTable) => {
            return {
              onClick: () => {
